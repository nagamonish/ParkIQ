...
from backend.models.schemas import CameraCreate, DetectionSettings, HomographyRequest, SlotUpdate
from backend.services.calibration import auto_detect_parking_slots
from backend.services.cloud_publisher import CloudPublisher
...
cloud_publisher = CloudPublisher(camera_manager)
...
# CORS: allow_credentials with a wildcard origin is rejected by browsers, so we
...
# Credentials may only be enabled when NO wildcard is present (browsers reject
# "*" + credentials), including the mixed "*,https://x" case.
...
async def update_camera_settings(camera_id: str, payload: DetectionSettings) -> dict[str, Any]:
    settings = camera_manager.update_settings(
        camera_id,
