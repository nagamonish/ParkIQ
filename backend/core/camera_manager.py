...
class CameraWorker:
    ...
    def update_settings(
        ...
        return self.detector.update_settings(iou_threshold, confidence_threshold, smoothing_window)
...
class CameraManager:
    ...
    def get_settings(self, camera_id: str) -> dict[str, Any] | None:
        worker = self.get_worker(camera_id)
        return worker.settings() if worker else None

    def update_settings(self, camera_id: str, **kwargs: Any) -> dict[str, Any] | None:
        worker = self.get_worker(camera_id)
        return worker.update_settings(**kwargs) if worker else None
