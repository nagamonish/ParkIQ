...
class ParkingDetector:
    ...
    def settings(self) -> dict[str, Any]:
        return {
            "iou_threshold": round(float(self.iou_threshold), 3),
    ...
    def update_settings(
        ...
        """Live-tune detection parameters without restarting the camera."""
        ...
        with self._slot_lock:
            ...
            if smoothing_window is not None:
                ...
                for slot in self.slots.values():
                    slot.smoothing_window = window
                    slot.vote_buffer = deque(slot.vote_buffer, maxlen=window)
                    slot.confidence_buffer = deque(slot.confidence_buffer, maxlen=window)
